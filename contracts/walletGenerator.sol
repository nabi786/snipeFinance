...
contract walletGenerator is Ownable {
    ...
    constructor(address tokenAddr){
        variableFee = 10**16;
        planDetails["1month"] = plan(10*10**18, 30);
        planDetails["year"] = plan(100*10**18, 60);
        token = IERC20(tokenAddr);
    }

    function payFee(uint amountWallets) public restrictionCheck(msg.sender){
        if(!qualifyForFreeAccess(msg.sender)) {
            token.transferFrom(msg.sender, address(this), amountWallets*variableFee);
            paidFeeRecords[msg.sender].push(variableFeeRecord(block.timestamp, amountWallets, amountWallets*variableFee));
        }
        else {
            paidFeeRecords[msg.sender].push(variableFeeRecord(block.timestamp, amountWallets, 0));
        }
    }

    function buyPlan(string memory _plan) public restrictionCheck(msg.sender) {
        token.transferFrom(msg.sender, address(this), planDetails[_plan].price);
    ...
    function getLastFeePaid(address addr) public view returns(variableFeeRecord memory){
        uint len = paidFeeRecords[addr].length;
        if(len == 0){
    ...
    function getPlanExpiry(address addr) public view returns(uint){
        return userPlan[addr].trySub(block.timestamp);
    }

    // new functions
    function setTknQtyForFreeAccess(uint256 _newTknQty) external onlyOwner {
        require(_newTknQty != 0, "invalid new quantity");
        tknQtyForFreeAccess = _newTknQty;
    }

    function qualifyForFreeAccess(address addr) public view returns(bool) {
        uint256 userTokenBalance = token.balanceOf(addr);
        if(userTokenBalance >= tknQtyForFreeAccess)
            return true;
        else
            return false;
    }
