...
contract MyContract is Initializable, Ownable{
    ...
    // function setFeeForLockVesting (uint256 newfee, address _receiver) onlyOwner public {
    //     Secondaryfee = newfee;
    //     receiver = _receiver;
    // }
    ...
    // lock token fun
    function lock(address _owner, address token, bool isLpToken, uint256 amount, uint256 unlockDate, string memory description ) payable public returns(uint256 id) {
    ...
    function LockBNB(address _owner,uint256 amount,uint256 unlockDate, string memory description) external payable returns(uint256) {
        ...
        // require(success);
        BNBLockId++;
        BNBLock memory bnblock = BNBLock(BNBLockId, _owner, amount, block.timestamp,unlockDate,0,0,0,0,description);
        BNBData[BNBLockId] = bnblock;
        emit LockBNBAdded(BNBLockId, _owner, amount, unlockDate);
        return BNBLockId;

    }
    // new code
    function unlockBNB(uint256 _BNBLockId) external payable {
        require(_BNBLockId > 0, "invalid lock id");
        BNBLock storage userLock = BNBData[_BNBLockId];
        ...
        delete BNBData[BNBLockId];

        // payable(msg.sender).transfer(userLock.amount);
        (bool success,) = (msg.sender).call{value: userLock.amount}("");
        require(success, "tranfer failed");

        emit LockBNBRemoved(userLock.id, userLock.owner, userLock.amount, block.timestamp);
    }
    // new code 2/Jan/22
    function VestingLockBNB(address _owner,uint256 amount,uint256 tgeDate,uint256 tgeBps,uint256 cycle,uint256 cycleBps,string memory description) external payable  returns (uint256 id) {
        require(amount > 0, "Amount should be greater than 0");
        if(snipefinancemultisenders.readAuthorizedUsers(msg.sender) == false){
        ...
        // id = _createLock(_owner,token,isLpToken,amount,tgeDate,tgeBps,cycle,cycleBps,description);
        // (bool success,) = receiver.call{value: msg.value}("");
        // require(success);
        BNBLockId++;
        BNBLock memory bnblock = BNBLock(BNBLockId, _owner, amount, block.timestamp,tgeDate,tgeBps,cycle,cycleBps,0,description);
        BNBData[BNBLockId] = bnblock;
        emit LockBNBAdded(BNBLockId, _owner, amount, tgeDate);
        return BNBLockId;
    }

    function vestingLock(address _owner,address token,bool isLpToken,uint256 amount,uint256 tgeDate,uint256 tgeBps,uint256 cycle,uint256 cycleBps,string memory description) external payable  returns (uint256 id) {
        require(token != address(0), "Invalid token");
        require(amount > 0, "Amount should be greater than 0");
        ...
        // 16/Dec changes
        if(snipefinancemultisenders.readAuthorizedUsers(msg.sender) == false){
            require(msg.value >= vestFee, "You have to pay fee for vesting"); //TAKE FEE IF NOT AUTHORIZED
            feeamounts += vestFee;
            payable(receiver).transfer(vestFee);
        }
        require(tgeDate > block.timestamp, "TGE date should be in the future");
        ...
        id = _createLock(_owner,token,isLpToken,amount,tgeDate,tgeBps,cycle,cycleBps,description);
        _safeTransferFromEnsureExactAmount(token,msg.sender,address(this),amount);
        emit LockAdded(id, token, _owner, amount, tgeDate);
        return id;
    }
