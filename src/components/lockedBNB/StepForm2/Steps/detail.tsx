...
import { getLockdBNBForBNBDetialPage } from '@app/utils/APIs/apis';
...
export const LockedBNBPageDetial: React.FC = () => {
  const { t } = useTranslation();
  const walletAddress = useParams();
  const [loading, setLoading] = useState(false);
  const [currentTokenState, setCurrentTokenState] = useState<[]>([]);
  const [unlockDate, setUnlockDate] = useState('');
  const [tokenSummary, setTokenSummary] = useState<[{ name: string; value: string }]>([
  ...
  useEffect(() => {
    const getCurrentTokenData = async () => {
      ...
      setLoading(true);
      var CurrentTokenData: any = await getLockdBNBForBNBDetialPage(walletAddressParm);
              ...
              return (
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }} key={index + 10}>
                  <Box
                    ...
                    key={index + 9}
                  >
                    {/* <Box sx={{marginTop : "30px", fontWeight : "300"}} kye={index+66}>{item.walletAddress}</Box> */}
                    <Box sx={{ marginTop: '30px', fontWeight: '300' }} key={index + 100}>
                      {item.walletAddress}
                    </Box>
                  </Box>
                  ...
                  >
                    <Link to={`/bnb/detail/${tokenSummary[0].value}/${item._id}`} key={index + 1}>
                      View
                    </Link>
                  </Box>
                </Box>
