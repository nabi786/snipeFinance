...
export const StepForm: React.FC = () => {
  ...
  const steps = [
    {
      title: <Typography sx={{ fontSize: '11px' }}>Choose Service</Typography>,
    },
    {
      title: <Typography sx={{ fontSize: '11px' }}>Pay and obtain service</Typography>,
    },
    {
      title: <Typography sx={{ fontSize: '11px' }}>Confirmation</Typography>,
    },
  ];
  ...
  return (
    ...
    >
      {/* <Steps labelPlacement="vertical" size="small" current={current} style={{ fontSize: '10px' }}>
        {steps.map((item) => (
          <Steps.Step style={{ fontSize: '10px' }} key={item.title} title={item.title} description="" />
        ))}
      </Steps> */}
      ...
      {/* <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', mt: '100px' }}>
        <Box>
